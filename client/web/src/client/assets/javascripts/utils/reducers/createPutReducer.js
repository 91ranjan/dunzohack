...
import { isActionLocal } from '../getActions';

export default function createPutReducer(config = {}) {
    ...
    return function(state, action) {
        ...
        if (filterId) {
            ...
            if (isActionPending(action.type)) {
                // Update item state
                updateItemState(itemState, filterId, ACTION_STATES.PENDING);
            } else if (isActionSuccess(action.type)) {
                // Update item state
                itemState = itemState.setIn([filterId, 'state'], ACTION_STATES.PENDING);

                // Update item
                itemState = itemState.setIn(
                    [filterId, 'version', 'synced'],
                    action.payload.response.getIn(itemPath)
                );
            } else if (isActionFailure(action.type)) {
                state = updateItemState(itemState, filterId);
            }
        } else {
            // REDUCING LIST STATE
            // PUT THE LIST ITEMS
            ...
            if (isActionPending(action.type)) {
                // POST List
                updateItemState(listState, serializedFilter, ACTION_STATES.PENDING);
            } else if (isActionSuccess(action.type)) {
